...
// 	token := NewTokenWithoutLine(tokenType, literal).Start(l.start + 1).End(l.pos)

// 	l.tokens = append(l.tokens, *token)
// }

// func (l *Lexer) getToken(tokenType TokenType, literal string) *Token {
// 	token := NewTokenWithoutLine(tokenType, literal).Line(l.line).Start(l.start + 1).End(l.col)
// 	return token
// }
